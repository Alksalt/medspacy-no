...
- Never redistribute NorSynthClinical or NorSynthClinical-PHI derivatives without written permission from the authors (Øvrelid, Brekke, Bråten) — those repos have no LICENSE file and are all-rights-reserved by default. NorMedTerm currently states CC BY 4.0 terms; use it only with attribution and a separate bundling decision. The own synthetic gold set stands alone if permission lags.
- State the synthetic-eval limitation plainly in any paper. No hedging, no minimizing.
- OSI license only — MIT or Apache-2.0.
- Do not gate the software release on a conference acceptance. JOSS is rolling, but submit only after production resources, public development history, release artifacts, docs, tests, paper.md, and evaluation evidence are ready.
